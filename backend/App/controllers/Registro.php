...
class Registro{
    ...
    public function index() {
        ...
        <title>
            Registro - VI Congreso Mundial de Patología Dual
        </title>
...
html;
    ...
    public function alerta($id, $parametro, $type_deposit, $name_register, $costo, $limit_pay, $reference_user, $modality,$email){
        ...
        if($parametro == 'add')
        {
            ...
            {
                $pay = 'Tarjeta de Crédito ó Débito';
                $name = $name_register;
                ...
                $message_pay = 'Nota importante: Incluya la referencia que le proporciona este sistema en el campo "Concepto
                de pago" según las instrucciones anteriores. La referencia de pago debe ingresarse en letra capital. No agregue espacios entre los nombres ni incluya ningún otro signo de puntuación, ya que
                esto puede afectar la confirmación de su transferencia bancaria';
                $amount = $costo." USD";
                $date_pay = $limit_pay;
                $reference = $reference_user;
                $account_number = '021180040158530967';
                $bank = 'HSBC';
                $name_association = 'Asociación Psiquiátrica Mexicana A.C';
            ...
            {
                $pay = 'Transferencia Electrónica';
                $name = $name_register;
                $addres = 'Periférico Sur No. 4194, Int. 104, Col.Jardines del Pedregal, CDMX, CP.01900 ';
                $message_pay = '';
                $amount = $costo." USD";
                $date_pay = $limit_pay;
                $reference = $reference_user;
